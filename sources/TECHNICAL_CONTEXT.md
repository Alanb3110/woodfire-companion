...
Logical fields include `view`, `mealTime`, `servings`, `completed`, `taskShifts`, `observations`, `rechecks`, `temperatureTarget`, `measurements`, `cookStartedAt`, `sessionId`, `sessionStartedAt`, `sessionServedAt`, `targetServingAt`, `activeTab`, `recipeId`, `recipeVersion` and `activeRecipeUrl`.
...
Older active records without `observations`/`rechecks` remain readable; defaults are additive (`[]` and `{}`). Compatible metadata is created from existing progress where possible.

`completed[stepId]` stores actual completion timestamps. `taskShifts[stepId]` stores explicit manual delay attached to a step. `rechecks[stepId]` stores only a pending observation deadline, not a rewritten task start time.
...
A not-ready observation records the observed state and a future `recheckDueAt`, but leaves the step incomplete and does not shift its historical start. A ready observation records the actual completion timestamp. The next-action card uses the pending recheck deadline when one exists.

This lets planner buffers absorb real cook variability before downstream work is moved. The existing +5/+10/+15 controls move a pending recheck deadline when the next unfinished step is awaiting recheck; otherwise they retain normal explicit-delay semantics.

Pork Belly `first-check` and turkey `finish-turkey` exercise the two current main cases.
...
A journal entry includes recipe identity/version/title, servings, target/actual service timestamps, temperature samples, structured observations, actual step completions, explicit delays, and baseline/final schedule timestamps.

Observation data is an additive Journal V1 field; old journal entries without it remain valid.
...
Observation-based completion uses the same actual timestamp path. A scheduled recheck by itself does not falsely complete or shift the step.

The +5/+10/+15 controls delay only the next unfinished step using `addStepDelay()` unless that step is awaiting recheck, in which case they move only the pending recheck deadline. Planner V1 decides what downstream work truly moves. Completed timestamps remain historical facts.
...
Observation controls do not automatically infer readiness from a measurement in V1; the cook still explicitly confirms the completion state.
...
Current dev version: `0.3.0-dev.5`.

Static shell/modules are listed in `APP_ASSETS`, including the meal-planner facade plus `js/observations.js` and `observations.css`. Executable recipe JSON is discovered from `recipes/index.json` and preloaded automatically for every `available` entry.
...
Coverage includes hardened recipe-contract fixtures, generic available-recipe acceptance, recipe-specific reference timelines where useful, library resolution, shopping/pre-cook, manifest-driven offline caching, version consistency, DOM contracts, dependency planner behavior, resource conflicts, buffer/service slippage, actual completion propagation, active-cook wiring, observation/recheck state transitions, journal observation snapshots, journal serialization/upsert/removal, service-milestone resolution and session/journal integration.
...
3. Make temperature tracking explicitly optional before adding recipes that do not benefit from core-temperature logging.
4. Add a flexible planning-window concept when a real recipe demonstrates the need.
5. Extend resource conflict handling beyond Woodfire only when real meals demonstrate a shared-resource collision.
6. Add journal notes/rating and JSON backup/import when useful.
7. Replace temporary CSS/emoji covers with local illustrated assets when visual direction is finalized.
8. Add richer resources such as user attention only when real meal plans demonstrate the need.
9. Add predictive ETA later from temperature/history with uncertainty, never false precision.
10. Add curated observation labels only if derived V1 controls prove insufficient for a real recipe.
