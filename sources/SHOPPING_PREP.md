...
Advance preparation is informative in this increment. It does not yet create planner nodes or automatically move the start time. If an advance-prep item later needs formal scheduling/dependencies, migrate it into planner semantics rather than duplicating schedule logic in the UI.
...
The initial persistence key is:

`woodfire-companion-shopping-v1`

State is keyed by recipe id and shopping-item id. Serving changes update quantities but retain checked identities because ingredient ids remain stable.
